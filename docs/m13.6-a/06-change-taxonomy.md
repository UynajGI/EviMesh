...
- `carriedActiveImpacts`：窗口开始前已记录、且截至 `asOf` 尚未被较新正式事件明确解决、关闭、撤回或替代的活跃影响集合。每项都必须保留其原始 `ResearchEvent`、影响对象与任何解决／替代事件；无法建立这一集合时不得输出 `quiet`。Finding 是 receipt-bound observation，当前没有 resolution/lifecycle 字段，不能用 Finding 的“未解决”状态推断该集合。
...
窗口内至少有一个明确影响当前对象或其下游使用范围的高影响事件，或 `carriedActiveImpacts` 中仍有同类影响，包括：

- Claim revision 进入 `refuted` 或 `retracted`；
- 当前 Frontier 的成员、依赖或可用性受到污染，或某 Claim revision 被移除、替换并明确影响当前阅读对象；
- 新增或升级为 `critical` 的 receipt-bound Finding observation，且关联目标 Claim revision、Verification receipt 或其明确下游，并有正式事件说明其影响范围；
...
至少展示作为主依据的 `eventId`、`eventType`、事件顺序或时间、`entity_type` 与对象 ID；对 Claim 必须带 `claimId + claimRevisionId`。按事件类型补充关联的 `findingId + receiptId`、`challengeId + challengeRevisionId + policyRevisionId`，或 `frontierSnapshotId + snapshotRevisionId + affected claimRevisionId`。Verification receipt 必须使用其实际记录的 `receiptId`、`claimId + claimRevision` 与 `contractId + contractRevision`；Policy 只有在实际 `PolicyEvaluation` 存在时才附 `evaluationId + policyId + policyRevision`。可用时同时提供 actor、parent event、hash、signature／包含证明和永久链接。主依据之外的同窗口事件不得被静默丢弃。
...
- 只有较新的正式事件明确解决、撤销或替代原 `critical` 影响时，才允许重新分类；重新分类必须同时链接原事件和解决/替代事件。若 `carriedActiveImpacts` 中仍有 critical 影响、有效的高影响 Challenge 或 Frontier 污染，保持 `critical`；不得以 Finding 的不存在的 resolution/lifecycle 字段判断降级。
...
窗口内至少有一个需要读者理解争议、阻塞或反向影响的事件，或 `carriedActiveImpacts` 中仍有同类影响，但未满足 `critical`，包括：

- Claim 或相关对象进入 `contested`；
- 新增或升级为 `major` 的 receipt-bound Finding observation，且有正式事件说明其影响范围；
...
补充文案必须说清对象和事实，例如：`新增一项 major Finding observation，关联 Verification receipt {receiptId}；请查看影响位置和 receipt-bound details。` 或 `出现新的反驳 Evidence，目标是 Claim revision {claimRevisionId}；这不单独决定 Claim 的真值。`

**ResearchEvent 与对象 provenance**

展示触发事件的 `eventId`、`eventType`、顺序／时间、目标对象 ID 与 revision；Evidence 变化附 `evidenceId + evidenceRevisionId + relationType + claimRevisionId`，Verification 变化附 `receiptId + claimId + claimRevision + contractId + contractRevision`，并且只有实际 PolicyEvaluation 存在时才附 `evaluationId + policyId + policyRevision`；Finding 变化附 `findingId + receiptId`，Challenge 变化附其稳定 ID、revision 和目标。事件 payload、actor、parent、hash、signature／包含证明和相关永久链接按可见性提供。
...
- `update` 不是对旧事件的清除。只有观察窗口内没有任何 qualifying event 且 `carriedActiveImpacts` 为空时，才可产生 `quiet`；“更新已读”或“更新较旧”不改变这一事实。
...
仅当同时满足以下条件：已声明 `asOf` 和明确 `window`；`objectScope`、`visibilityScope` 和事件排序完整；在窗口内没有观察到属于范围的 qualifying event；并且 `carriedActiveImpacts` 为空。默认文案使用七日窗口，但必须把实际窗口写出来。
...
- `quiet` 只在没有任何 qualifying event 且没有 carried active impact 时成立；任意一个合格事件或活跃影响都会使分类至少变为 `update`，或按其影响升级。
...
1. 固定 `asOf`、`window`、`objectScope` 和 `visibilityScope`，确认事件输入可见且可排序；从窗口开始前的正式事件建立 `carriedActiveImpacts`，并保留每项的原始和解决／替代事件。
2. 选取窗口内与对象有明确关系的 qualifying event，并保留每个事件的对象 ID 与 revision；Finding 仅作为 receipt-bound observation，不得以不存在的 resolution/lifecycle 谓词筛选。
3. 按窗口事件和 `carriedActiveImpacts` 的影响匹配 `critical`、`attention`、`update`；只有两者皆空且输入完整，才匹配 `quiet`。
