...
| 验证结果 | 按 `VerificationReceipt.outcome` 展示适用于该 Claim revision 的结果（如 `supports`、`refutes`、`qualifies`、`inconclusive`），并保留每条回执，而不是只给汇总。 | 输入集为记录的 `claimId + claimRevision` 等于目标 revision、且在 `asOf` 前可见的 `VerificationReceipt`；按每条 receipt 的 Contract 语境分组，Policy 仅在实际引用的 `PolicyEvaluation` 存在时另行分组。 | 每条结果：`receiptId` + `claimId` + `claimRevision` + `contractId` + `contractRevision`；如有实际 PolicyEvaluation，再附 `evaluationId` + `policyId` + `policyRevision`；附 `eventId`（如有）。 | 没有回执只表示该输入集为空；结果字段缺失为“未知”。 |
| 实现与数据独立性 | 对每条适用的验证回执陈述 implementation independence 与 data independence 的原始分类或“未提供”；不得合成为可信度。 | 与“验证结果”完全相同的 receipt 输入集和 `asOf`；不得混入其他 Claim revision 的 profile。 | `receiptId` + `claimId` + `claimRevision` + `contractId` + `contractRevision`；若实际引用 PolicyEvaluation，另附其 `evaluationId` + `policyId` + `policyRevision`。 | 该验证类型不评估独立性为“不适用”；未记录为“未提供／未知”。 |
| Receipt-bound Finding 观察的严重程度 | 对关联回执的 Finding observations 按 `severity` 陈述，并列出达到所陈述等级的全部 Finding，避免一个标签掩盖来源。不得把 Finding 说成“未解决”或“已解决”。 | 输入集为关联上述 receipt、其记录的 `claimId + claimRevision` 等于目标 revision、且 `createdAt` 不晚于 `asOf` 的 Finding；严重程度仅为 `critical`、`major`、`warning`、`note`。 | 每个 Finding：`findingId` + `receiptId` + `severity` + `code` + `createdAt`。 | 输入集为空时写“在已声明 receipt 输入集和 asOf 内未观察到 Finding”；Finding resolution/lifecycle 目前未建模，不能用启发式补充。 |
...
| Policy 与 Contract 修订 | 对每项来自 Verification、Challenge 或 Frontier 的结论，显示其实际记录的 Contract revision；Policy 只在实际存在的 `PolicyEvaluation` 时显示，且不得把不同规则版本合并为一个结论。 | 输入集为摘要中实际引用的 receipts、challenges、snapshots，以及实际引用的 PolicyEvaluation；截至各对象的固定记录及摘要 `asOf`。 | 回执结论附 `receiptId` + `contractId` + `contractRevision`；Policy 结论另附 `evaluationId` + `policyId` + `policyRevision`；并附 receipt、snapshot 或 event 锚点。 | 某对象不使用该规则为“不适用”；字段未记录为“未提供”。 |
