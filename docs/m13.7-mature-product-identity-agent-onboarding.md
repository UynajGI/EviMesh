...
## 12. 2026-08-10 实现状态与证据边界

M13.7-A 的审计、产品契约和概念测试材料已经落入仓库；`/prototypes/m13-7-a` 也已作为可验证的说明性原型加入。该路由只使用本地 fixture，不能登录、保存、关联身份、连接 Agent 或访问真实研究数据，因此它不是已上线产品，也不构成这些能力可用的证据。

- A07 仍为外部技术门禁：在获得书面授权的 ORCID Sandbox client 和隔离的 Supabase 测试环境前，不得声称 OAuth/OIDC、回调、token exchange、账号关联、碰撞恢复或缺失邮箱路径已经验证。见 [`m13.7-a/07-orcid-supabase-spike-adr.md`](m13.7-a/07-orcid-supabase-spike-adr.md)。
- A10 仍为外部研究门禁：在按 A09 计划完成至少五位已同意目标用户的会话前，不得声称概念或导航研究结论。参与者资料和同意记录必须保留在批准的研究系统外部，仓库只保留去标识化结论。见 [`m13.7-a/09-prototype-test-plan.md`](m13.7-a/09-prototype-test-plan.md) 和 [`m13.7-a/10-concept-navigation-test-report.md`](m13.7-a/10-concept-navigation-test-report.md)。
- 因此，M13.7-A 及后续 M13.7-B 至 M13.7-F 均不得因该 fixture 原型而标记为完成；后续实现仍须经过各自的契约、测试和生产门禁。
