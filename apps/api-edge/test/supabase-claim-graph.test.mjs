...
test("reads bounded upstream and downstream Claim graphs from active dependency relations", async () => {
  ...
  assert.deepEqual(upstream.map((node) => node.claimId), ["claim-a", "claim-root"]);
  assert.deepEqual(downstream.map((node) => node.claimId), ["claim-a", "claim-child"]);
  assert.equal(downstream[1].state, "dependency_tainted");
});
