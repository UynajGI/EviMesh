...
export default function ProjectDetailPage({ params }) {
  ...
  useEffect(() => {
    if (!projectId) return;
    Promise.all([request(`/projects/${projectId}`), request(`/questions?projectId=${projectId}&limit=6`), request(`/tasks?projectId=${projectId}&limit=6`), request(`/projects/${projectId}/frontier/latest`)])
      .then(([project, questions, tasks, frontier]) => setData({ project, questions: questions.items ?? [], tasks: tasks.items ?? [], frontier: frontier.frontier }))
