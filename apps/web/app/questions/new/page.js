...
export default function NewQuestionPage() {
  ...
  return <main className="mx-auto max-w-3xl px-6 py-16">
    ...
    <form onSubmit={step === 1 ? continueToScope : step === 2 ? continueToProgress : step === 3 ? continueToPermissions : reviewQuestion} className="mt-10 space-y-6 rounded-xl border border-border bg-card p-6 shadow-sm">
      ...
      {step === 1 ? <>
        <label className="block"><span className="text-sm font-semibold">Question ID</span><Input required className="mt-2" aria-label="Question ID" placeholder="A stable question ID" value={draft.questionId} onChange={(event) => update('questionId', event.target.value)} /></label>
        <label className="block"><span className="text-sm font-semibold">Project ID</span><Input required className="mt-2" aria-label="Project ID" placeholder="The project that owns this question" value={draft.projectId} onChange={(event) => update('projectId', event.target.value)} /></label>
        ...
        <label className="block"><span className="text-sm font-semibold">Research contract ID</span><Input required aria-label="Research contract ID" className="mt-2" placeholder="Immutable contract reference" value={draft.contractId} onChange={(event) => update('contractId', event.target.value)} /></label>
        <label className="block"><span className="text-sm font-semibold">Contract revision</span><Input required aria-label="Contract revision" className="mt-2" min="1" type="number" value={draft.contractRevision} onChange={(event) => update('contractRevision', event.target.value)} /></label>
