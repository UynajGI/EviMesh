...
export default function NewQuestionPage() {
  ...
  const [draft, setDraft] = useState({ title: '', statement: '', value: '', scope: '', exclusions: '', progress: '', falsification: '' });
  ...
  return <main className="mx-auto max-w-3xl px-6 py-16">
    ...
    <p className="mt-10 text-sm font-bold uppercase tracking-[0.18em] text-primary">Question submission · {step === 1 ? 'Step 1 of 4' : step === 2 ? 'Step 2 of 4' : 'Step 3 of 4'}</p>
    <h1 className="mt-3 text-5xl font-semibold tracking-tight">{step === 1 ? 'Frame the question' : step === 2 ? 'Define the scope' : 'Describe progress and falsification'}</h1>
    <p className="mt-5 max-w-2xl leading-7 text-muted-foreground">{step === 1 ? 'State what you want to learn and why the answer matters. Later steps will define the scope, progress, and safeguards.' : step === 2 ? 'Make the investigation bounded and reproducible by stating what is included and what is explicitly out of scope.' : 'Record what is already known and the evidence that would show the question should be rejected or revised.'}</p>
    <form onSubmit={step === 1 ? continueToScope : step === 2 ? continueToProgress : continueToPermissions} className="mt-10 space-y-6 rounded-xl border border-border bg-card p-6 shadow-sm">
      {step === 1 ? <>
      ...
      <div className="flex items-center justify-between gap-4"><Button type="button" variant="outline" onClick={() => { setSaved(false); setStep(Math.max(1, step - 1)); }} disabled={step === 1}>Back</Button><Button type="submit">{step === 1 ? 'Continue to scope' : step === 2 ? 'Continue to progress' : 'Continue to permissions'}</Button></div>
      {saved && <p role="status" className="text-sm text-primary">Step 3 is complete. Permission fields are ready for the next step.</p>}
    </form>
  </main>;
}
