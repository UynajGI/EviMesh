...
- `create_claim` / `record_run` 只产出本地草稿对象，不触网；两者从 `~/.evimesh` 的活动 Ed25519 身份派生 `did:key` 归属，忽略调用方伪造的 actor/signature 字段。Run 草稿会对无签名正文生成可验证的执行签名，并携带 source、不可变容器摘要和环境/硬件，返回前按协议 schema 校验。
