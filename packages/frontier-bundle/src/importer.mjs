...
function collectBundleDocuments(files) {
  ...
  return { claims, evidence, receipts, events, checkpoints, frontier: parseJson(files, BUNDLE_FILES.frontier) };
}
...
export async function importFrontierBundle({ repository, files, platformKey = null } = {}) {
  ...
  const { claims, evidence, receipts, events, checkpoints, frontier } = collectBundleDocuments(files);
  ...
  return repository.withTransaction(async (transaction) => {
    ...
    for (const { document } of evidence) {
      ...
      if (document.claimId) {
        await transaction.insertEvidenceClaimLink?.({ evidenceId: document.evidence.evidenceId, claimId: document.claimId, claimRevision: document.claimRevision, relationType: document.evidence.relationType ?? "supports" });
      }
    }
